...
# Load CSV data generated by Experiment 1 in run_simulations.R
exp1 <- read.csv(file.path(results_dir, "exp1_cstar_magnitude.csv"))

# Plot 1: loss reduction comparison
# Reshape data from wide to long format using 'melt' so ggplot can group line colours by the 'method' column
...
# Initialize the ggplot object, mapping the x-axis to the multiplier and the y-axis to the loss reduction
p1 <- ggplot(exp1_long, aes(x = multiplier, y = loss_reduction, color = method)) +
  geom_line(size = 1.2) + # Draw thick line charts
  geom_point(size = 2.5) + # Add points at each data interval
  scale_color_manual(values = c("reduction_diim" = "#3498DB", "reduction_pca" = "#2ECC71"),
    labels = c("DIIM Sectors", "PCA Sectors")) + # Custom hex colours (DIIM Blue, PCA Green)
  labs(title = "Experiment 1: Loss Reduction vs c* Magnitude",
       x = "c* Multiplier", y = "Economic Loss Reduction", color = "Method") +
  theme_minimal() + # Apply clean white background formatting
...
# Plot 2: PCA advantage (positive = PCA better)
# Create a new column explicitly measuring the difference between the two models
...
p1b <- ggplot(exp1, aes(x = multiplier, y = advantage_pca)) +
  ...
  # Draw a horizontal line exactly at 0 where PCA performance breaks even against DIIM
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 0.8) +
  
  # Shade the top half (positive outcome for PCA) in faint green
  annotate("rect", xmin = min(exp1$multiplier), xmax = max(exp1$multiplier),
           ymin = 0, ymax = Inf, fill = "#2ECC71", alpha = 0.1) +
           
  # Shade the bottom half (negative outcome for PCA) in faint red
  annotate("rect", xmin = min(exp1$multiplier), xmax = max(exp1$multiplier),
...
# Compute PCA advantage over DIIM explicitly
exp3$advantage_pca <- exp3$reduction_pca - exp3$reduction_diim

# Create grouped bar charts (position = "dodge") distinguishing different q0 patterns side-by-side
p3 <- ggplot(exp3, aes(x = factor(c_star_multiplier), y = advantage_pca, fill = q0_pattern)) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.85) +
  
  # Highlight the zero break-even line in red
...
# Load Experiment 4's massive grid search dataset
...
# Heatmap 1: Continuous gradient PCA advantage heat block
# Red regions represent situations where DIIM outperformed PCA (highly negative advantage). 
# Green regions outline where PCA structurally dominated.
p4 <- ggplot(exp4, aes(x = factor(c_star_multiplier), y = factor(lockdown_duration), fill = advantage_pca)) +
  geom_tile(color = "white", size = 0.3) + # Plot boxes partitioned by white outlines
...
# Scatter: c_star Gini vs q0 Gini, coloured by winner
...
# Logistic regression: which features predict PCA winning?
...
# Exp 1: which c* multipliers does PCA win at?
...
# Exp 2: which distribution patterns does PCA win at?
