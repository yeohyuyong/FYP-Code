...
for (i in 1:n_mc) {
    ...
    mc_results[[i]] <- data.frame(
        sim_id = i,
        q0_features,
        reduction_diim = experiment_results$reduction_diim,
        reduction_pca  = experiment_results$reduction_pca,
        pca_advantage  = experiment_results$reduction_pca - experiment_results$reduction_diim,
        pca_wins = experiment_results$pca_wins,
        overlap  = experiment_results$overlap,
...
cat(sprintf("Total: PCA wins %d / %d (%.1f%%)\n",
...
cat("Full Model (all 8 q0 features):\n")
...
cat(sprintf("q0_top5_overlap threshold (P=0.5): %.4f\n", threshold_overlap))
...
cat(sprintf("q0_pca_share: AUC=%.4f, Optimal threshold (Youden)=%.4f\n", auc_share, optimal_threshold_share))
cat(sprintf("  At optimal: TPR=%.3f, FPR=%.3f, Accuracy=%.3f, F1=%.3f\n",
...
cat(sprintf("  Max accuracy threshold: %.4f (accuracy=%.3f)\n",
...
sweep_results <- data.frame(
    target_share = numeric(), actual_share = numeric(),
    pca_wins = logical(), pca_advantage = numeric(),
    stringsAsFactors = FALSE
)
...
p1 <- ggplot(win_rate_by_share, aes(x = q0_pca_share, y = pca_win_rate)) +
    ...
    { if (!is.na(transition_share)) geom_vline(xintercept = transition_share, linetype = "dashed", color = "#3498DB", size = 0.8) } +
    { if (!is.na(transition_share)) annotate("text", x = transition_share + 0.02, y = 0.55, label = sprintf("Threshold = %.2f", transition_share), color = "#3498DB", fontface = "bold", hjust = 0) } +
    annotate("rect", xmin = 0.10, xmax = ifelse(is.na(transition_share), 0.70, transition_share),
             ymin = -Inf, ymax = Inf, fill = "#E74C3C", alpha = 0.05) +
    annotate("rect", xmin = ifelse(is.na(transition_share), 0.70, transition_share),
             xmax = 0.70, ymin = -Inf, ymax = Inf, fill = "#2ECC71", alpha = 0.05) +
    scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
    scale_x_continuous(labels = scales::percent) +
    labs(title = "Decision Boundary: PCA Win Rate vs q0 Share in PCA Sectors",
...
p2 <- ggplot(roc_all, aes(x = fpr, y = tpr, color = feature)) +
    ...
    scale_color_manual(
        values = c("q0_pca_share" = "#E74C3C", "q0_mean_ratio" = "#3498DB", "q0_top5_overlap" = "#2ECC71"),
        labels = c(sprintf("q0_pca_share (AUC=%.3f)", auc_share),
                   sprintf("q0_mean_ratio (AUC=%.3f)", auc_ratio),
                   sprintf("q0_top5_overlap (AUC=%.3f)", auc_overlap)),
        name = "Feature") +
    labs(title = "ROC Curves: q0-Based Predictors of PCA Outperformance",
...
p3 <- ggplot(mc_df, aes(x = share_bin, y = pca_advantage, fill = pca_wins)) +
    ...
    scale_fill_manual(values = c("TRUE" = "#2ECC71", "FALSE" = "#E74C3C"),
                      labels = c("TRUE" = "PCA Wins", "FALSE" = "DIIM Wins"), name = "") +
    labs(title = "PCA Advantage by q0 PCA Share Bin",
         subtitle = "Positive = PCA reduces more loss than DIIM",
         x = "q0 PCA Share Bin", y = "PCA Advantage (reduction_PCA - reduction_DIIM)") +
    theme_minimal() +
    theme(plot.title = element_text(face = "bold", size = 13),
...
p4 <- ggplot(mc_df, aes(x = q0_pca_share, y = q0_mean_ratio, color = pca_wins)) +
    ...
    { if (!is.na(threshold_share)) geom_vline(xintercept = optimal_threshold_share, linetype = "dashed", color = "black", size = 0.8) } +
    scale_color_manual(values = c("TRUE" = "#2ECC71", "FALSE" = "#E74C3C"),
                       labels = c("TRUE" = "PCA Wins", "FALSE" = "DIIM Wins"), name = "") +
    labs(title = "Decision Space: q0_pca_share vs q0_mean_ratio",
         x = "q0 PCA Share", y = "q0 Mean Ratio (PCA / non-PCA)") +
    theme_minimal() +
    theme(plot.title = element_text(face = "bold", size = 13), legend.position = "bottom")
...
if (!is.na(transition_share)) cat(sprintf("  Systematic sweep transition (50%% win): %.4f\n", transition_share))
if (!is.na(high_threshold)) cat(sprintf("  90%% confidence threshold:              %.4f\n", high_threshold))
...
cat(sprintf("  Rule 2: Use PCA if mean(q0[PCA]) / mean(q0[non-PCA]) > %.2f\n", optimal_threshold_ratio))
cat(sprintf("  Rule 3: Use PCA if >= %d of the top-5 inoperable sectors are PCA sectors\n", ceiling(optimal_threshold_ovl)))

cat("\nFULL MODEL (for highest accuracy):\n")
sig_vars <- summary(logit_full)$coefficients[, 4] < 0.05
sig_names <- names(which(sig_vars[-1]))
