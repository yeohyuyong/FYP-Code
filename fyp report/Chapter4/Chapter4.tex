...
This distinction is important because the DIIM solution simplifies to $q(t) = e^{-K(I-A^{*})t}\, q(0)$ when $c^{*} = 0$ (Equation~\eqref{eq:diim-solution-homogeneous}), meaning the disruption dynamics are governed entirely by the structure of the economy ($A^{*}$) and the initial shock ($q(0)$).
...
Figure~\ref{fig:manpower-economic-loss} presents the cumulative economic loss evolution. The 15-sector model reveals that even sectors with moderate inoperability can accumulate substantial losses if their baseline output is large (e.g., manufacturing, financial services).
...
Although both scenarios operate at the same 15-sector level, the manpower scenario is structurally distinct because the absence of a demand shock ($c^* = 0$) means the entire disruption propagation is driven by the production network alone. Furthermore, the 2022~IO table captures post-pandemic structural shifts in the economy. These structural differences have important implications for the effectiveness of simplified key sector identification methods, as explored in the following chapters.
