...
This study evaluates five cheap methods---Total Output, PCA $\times$ xi, PageRank $\times$ xi, BL $\times$ xi, and FL $\times$ xi---against the DIIM benchmark through Monte Carlo simulation (2\,000 random $q_0$ vectors per scenario). Performance is measured by the ratio of economic loss reduction achieved by each cheap method relative to DIIM. For the 15-sector COVID scenario, the best structural methods (Total Output, PageRank $\times$ xi, BL $\times$ xi, and FL $\times$ xi) match DIIM in 89.8\% of cases, and PCA $\times$ xi matches in 86.6\%. For the 107-sector manpower scenario, FL $\times$ xi and Total Output match in 55.4\% of cases, while PCA $\times$ xi drops to 38.8\%. Logistic regression is used to derive decision rules that predict, from observable $q_0$ features, when each cheap method will perform adequately.
