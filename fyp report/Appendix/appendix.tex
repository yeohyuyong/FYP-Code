...
\end{tabular}
\caption{Cross-section of intermediate input flow matrix, \$ million (COVID-19)}
...
The manpower disruption scenario uses the 2022 Singapore IO table with 15 sectors. Scenario parameters are summarised in Table~\ref{tab:scenario-comparison} (Chapter~4). The mean initial inoperability across the 15~sectors is 0.08165, and the maximum is 0.289 (Sector~15: Other Community, Social \& Personal Services). The PCA $\times\, x_i$ top-5 sectors are 2, 5, 4, 7, and 8.
...
\begin{itemize}
    ...
    \item \texttt{pca.R}: PCA on the influence matrix $H = AL$ and sector clustering
    \item \texttt{simulations/decision\_rules.R}: Monte Carlo framework, logistic regression, and decision boundary rules
    \item \texttt{simulations/enhanced\_comparison.R}: Varying top-$k$ comparison of all five simplified methods against the DIIM benchmark
