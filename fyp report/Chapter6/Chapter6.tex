...
\begin{enumerate}
    \item \textbf{For aggregated economies ($\leq$ 20 sectors)}: Simple baselines like Total Output or structurally-weighted methods (FL $\times$ xi, PCA $\times$ xi) can be used as primary screening tools. With close rates near 60--90\%, the risk of significant underperformance is manageable, though performance varies with the shock structure ($c^*$).
    \item \textbf{Shock structure matters}: The key driver of cheap method effectiveness at the same sectoral resolution is the shock structure. When both supply and demand shocks are present ($c^* > 0$, as in COVID-19), the demand shock anchors the disruption and cheap methods achieve close rates near 90\%. Under pure supply shocks ($c^* = 0$, as in manpower disruption), close rates drop to about 60\%, as the DIIM's adaptive ranking has more room to outperform fixed rankings.
