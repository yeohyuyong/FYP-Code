...
\subsection{PageRank $\times$ xi}

The interdependency matrix $A^*$ is treated as the weighted adjacency matrix of a directed graph. The PageRank algorithm is applied to compute a structural centrality score $PR_i$ for each sector, which measures importance based on the structure of incoming links with a damping factor. This centrality score is then weighted by the sector's total output:
...
\subsection{Backward Linkage $\times$ xi (BL $\times$ xi)}
...
\subsection{Forward Linkage $\times$ xi (FL $\times$ xi)}
