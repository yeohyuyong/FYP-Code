...
DIIM <- function(q0, A_star, c_star, x, lockdown_duration, total_duration, key_sectors = NULL, risk_management = 1, days_in_year = 366) {
  ...
  return(
    list(
      inoperability_evolution = inoperability_evolution, # Matrix of daily inoperability levels per sector
      EL_evolution = EL_evolution, # Matrix of cumulative economic loss over time per sector
      EL_end = EL_end, # Vector of total economic loss per sector at the final time step
      total_economic_loss = total_economic_loss # Total economic loss summed across all sectors
    )
  )
}
